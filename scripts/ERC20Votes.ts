...
async function main() {
    ...
    await mintTx.wait();
    console.log(
        `Minted ${MINT_VALUE.toString()} decimal units to accounts ${accounts[1].address}/n`
    );
    ...
    console.log(
        `Account ${accounts[0].address} has ${votes.toString()} units of voting power before self delegating/n`
    );
    ...
    console.log(
        `Account ${accounts[0].address} has ${votesAfter.toString()} units of voting power before self delegating/n`
    );
    ...
    console.log(
        `Account ${accounts[0].address} has ${votesAfterTransfer.toString()} units of voting power before self delegating/n`
    );
    const vote2AfterTransfer = await contract.getVotes(accounts[2].address);

    // Check past voting power 
    const lastBlock = await ethers.provider.getBlock("latest");
    console.log(`Current block number is ${lastBlock.number}/n`);
    const pastVotes = await contract.getPastVotes(accounts[1].address, lastBlock.number - 1);
    console.log(
        `Account ${accounts[1].address} has ${votesAfter.toString()} units of voting power before self delegating/n`
    );
